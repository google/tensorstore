...
def btc_protobuf(
    ...
    *,
    ...
    cmake_depends: Iterable[CMakeTarget] | None = None,
    flags: list[str] | None = None,
    output_dir: str | None = None,
    import_targets: Iterable[CMakeTarget] | None = None,
):
...
def plugin_generated_files(
  ...
  def _mktuple(ext: str) -> tuple[TargetId, pathlib.PurePath]:
    ...
    # The output directory accounts for strip_import_prefix so it doesn't
    # need to be considered and propagated as a PUBLIC target include in CMake.
    if proto_library_provider.strip_import_prefix:
      strip_prefix = proto_library_provider.strip_import_prefix
      if strip_prefix.startswith("/"):
        strip_prefix = strip_prefix[1:]
      if package_name == strip_prefix:
        package_name = ""
      elif package_name.startswith(strip_prefix + "/"):
        package_name = package_name[len(strip_prefix) + 1 :]

    generated_path = binary_dir.joinpath(package_name, f"{target_name}{ext}")
...
def aspect_genproto_library_target(
  ...
  aspect_deps: OrderedSet[TargetId] = OrderedSet()
  ...
  aspect_srcs: OrderedSet[TargetId] = OrderedSet()
  ...
  aspect_dir = repo.replace_with_cmake_macro_dirs([output_dir])[0]
  ...
  common_options = handle_cc_common_options(
      _context,
      _src_required=True,
      add_dependencies=OrderedSet(srcs_collector.add_dependencies()),
      srcs=aspect_srcs,
      deps=aspect_deps,
      includes=None,
