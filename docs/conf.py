...
nitpick_ignore = [
    # The Sphinx C++ domain generates bogus undefined reference
    # warnings for every C++ namespace that is mentioned in the
    # documentation. All such namespaces need to be listed here in
    # order to silence the warnings.
    ...
    # Due to lack of type alias support in Sphinx and Sphinx-immaterial.
    ('py:obj', 'tensorstore.T'),
    # https://github.com/sphinx-doc/sphinx/pull/13808
    ('py:class', 'types.EllipsisType'),
    ('py:class', 'numpy.typing.ArrayLike'),
    ('py:class', 'slice[typing.SupportsIndex | None]'),
    (
        'py:class',
        (
            'slice[typing.SupportsIndex |'
            ' collections.abc.Sequence[typing.SupportsIndex | None] | None]'
        ),
    ),
]
...
python_type_to_xref_mappings = {
    **{
        name: TypeXrefTarget('py', 'obj', name, name.split('.')[-1])
        for name in [
...
external_cpp_references = {
    ...
    'absl::StrFormat': {
        'url': 'https://abseil.io/docs/cpp/guides/format',
        'object_type': 'function',
        'desc': 'C++ function',
    },
    'std::move': {
...
def setup(app):
  ...
  # Exclude the `__class_getitem__` members of `Future` and `Promise`
  # that are solely for creating generic alias types.
  def _autodoc_skip_member(app, what, name, obj, skip, options) -> None | bool:
    del app, what, skip, options  # Unused.
    if name == '__class_getitem__' and not obj.__doc__:
      return True
    return None
