...
TEST(ValidateEndpointTest, EmptyBucket) {
  ...
  EXPECT_THAT(ValidateEndpoint("", {}, "https://mybucket.cwobject.com", {}),
              ::testing::VariantWith<absl::Status>(absl::OkStatus()));

  // Empty bucket + endpoint + host_header + region => resolved.
  EXPECT_THAT(ValidateEndpoint("", "us-east-1", "https://mybucket.cwobject.com",
...
TEST(ResolveEndpointRegion, EmptyBucket) {
  ...
  S3EndpointRegion ehr;
  TENSORSTORE_ASSERT_OK_AND_ASSIGN(
      ehr, ResolveEndpointRegion("", "https://mybucket.cwobject.com", {},
                                 mock_transport)
               .result());
