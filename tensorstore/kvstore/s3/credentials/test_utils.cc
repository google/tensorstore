...
Future<internal_http::HttpResponse> EC2MetadataMockTransport::IssueRequest(
  const internal_http::HttpRequest& request,
  absl::Cord payload,
...
absl::flat_hash_map<std::string, internal_http::HttpResponse> DefaultEC2MetadataFlow(
  ...
  return absl::flat_hash_map<std::string, internal_http::HttpResponse>{
    ...
    {"GET http://169.254.169.254/latest/meta-data/iam/",
      internal_http::HttpResponse{200, absl::Cord{"info"},
                  {{"x-aws-ec2-metadata-token", api_token}}}},
    {"GET http://169.254.169.254/latest/meta-data/iam/security-credentials/",
      internal_http::HttpResponse{200, absl::Cord{"mock-iam-role"},
                  {{"x-aws-ec2-metadata-token", api_token}}}},
    {"GET "
      "http://169.254.169.254/latest/meta-data/iam/security-credentials/"
      "mock-iam-role",
      internal_http::HttpResponse{200, absl::Cord(
        absl::StrFormat(R"({
                      "Code": "Success",
                      "AccessKeyId": "%s",
                      "SecretAccessKey": "%s",
                      ...
                      "Expiration": "%s"
                        ...
                        absl::FormatTime("%Y-%m-%d%ET%H:%M:%E*S%Ez",
                                        expires_at, absl::UTCTimeZone()))),
                  {{"x-aws-ec2-metadata-token", api_token}}}}};
}
