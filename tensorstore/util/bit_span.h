...
void CopyBits(const U* source, ptrdiff_t source_offset, T* dest,
              ptrdiff_t dest_offset, ptrdiff_t size) {
...
/// `ptrdiff_t`), and a `size` (of type `ptrdiff_t`).  Bit `i` (where
...
class BitSpan {
 ...
 public:
  ...
  constexpr BitSpan(T* base TENSORSTORE_ATTRIBUTE_LIFETIME_BOUND,
  ...
  constexpr ptrdiff_t offset() const { return begin().offset(); }
  ...
  template <typename U, ptrdiff_t E, int&... ExplicitArgumentBarrier,
            typename X = T>
