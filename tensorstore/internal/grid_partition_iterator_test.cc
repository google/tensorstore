...
using ::tensorstore::internal_grid_partition::PartitionIndexTransformIterator;
...
std::vector<R> GetPartitionsManual(
    const std::vector<DimensionIndex>& grid_output_dimensions,
    const std::vector<Index>& grid_cell_shape, IndexTransformView<> transform) {
  std::vector<R> results;
  ...
  while (!iterator.AtEnd()) {
    results.emplace_back(
        std::vector<Index>(iterator.output_grid_cell_indices().begin(),
                           iterator.output_grid_cell_indices().end()),
...
// Tests that an empty input shape is handled correctly.
TEST(PartitionIndexTransformOverRegularGrid, EmptyDimension) {
  const auto results = GetPartitions({0, 1}, {3, 3},
...
TEST(PartitionIndexTransformOverRegularGrid, SingleStridedDimensionManual) {
  const auto results =
      GetPartitionsManual({0}, {10},
                          IndexTransformBuilder<>(1, 1)
                              .input_origin({-4})
                              .input_shape({6})
                              .output_single_input_dimension(0, 5, 3, 0)
...
TEST(PartitionIndexTransformOverRegularGrid,
     IndexArrayAndStridedDimensionsManual) {
  const auto results = GetPartitionsManual(
      {0, 1}, {10, 8},
      IndexTransformBuilder<>(2, 2)
          .input_origin({-4, 100})
          .input_shape({6, 3})
