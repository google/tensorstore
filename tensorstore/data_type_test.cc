...
TEST(ElementOperationsTest, UnsignedIntCompareIdentical) {
  ...
  // SimpleElementwiseFunction.
  EXPECT_EQ(
      0, compare_identical[IterationBufferKind::kStrided](
             nullptr, 0, IterationBufferPointer{arr1, sizeof(unsigned int) * 2},
             IterationBufferPointer{arr2, sizeof(unsigned int)},
             /*status=*/nullptr));
  EXPECT_EQ(
      2, compare_identical[IterationBufferKind::kStrided](
             nullptr, 2, IterationBufferPointer{arr1, sizeof(unsigned int) * 2},
             IterationBufferPointer{arr2, sizeof(unsigned int)},
             /*status=*/nullptr));
  EXPECT_EQ(
...
TEST(ElementOperationsTest, FloatCompareIdentical) {
  ...
  const auto& compare_identical =
      r->compare_equal[static_cast<size_t>(EqualityComparisonKind::identical)]
          .array_array;
  EXPECT_EQ(1, compare_identical[IterationBufferKind::kStrided](
  ...
  EXPECT_EQ(3, compare_identical[IterationBufferKind::kStrided](
                   nullptr, 3, IterationBufferPointer{arr1, sizeof(float) * 2},
                   IterationBufferPointer{arr2, sizeof(float) * 2},
                   /*status=*/nullptr));

  EXPECT_EQ(2, compare_identical[IterationBufferKind::kStrided](
...
TEST(ElementOperationsTest, Class) {
  ...
  EXPECT_EQ(0, compare_equal[IterationBufferKind::kStrided](
