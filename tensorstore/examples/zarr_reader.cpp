...
void read_ometiff_data()
{
  ...
  TENSORSTORE_CHECK_OK_AND_ASSIGN(auto store, tensorstore::Open({{"driver", "zarr"},
                            ...
                            {"kvstore", {{"driver", "file"},
                                         {"path", "/mnt/hdd8/axle/data/nyxus_zarr_test/eastman-plate01-Seg-omezarr/eastman-plate01-Seg-omezarr/p01_x01_y01_wx0_wy0_c1.ome.zarr/0"}}
                            }},
                            context,
  ...
  auto array = tensorstore::AllocateArray<tensorstore::float32_t>({1,1,1,10, 10});
  ...
  for (int i=0; i<100; i++){
    ...
    tensorstore::Read(store | 
                    ...
                    tensorstore::Dims(3).ClosedInterval(0,9) |
                    tensorstore::Dims(4).ClosedInterval(0,9) ,
                    array).value();
    PrintCSVArray(array);
