...
#ifndef THIRD_PARTY_PY_TENSORSTORE_UFUNCS_H_
#define THIRD_PARTY_PY_TENSORSTORE_UFUNCS_H_
...
#include "tensorstore/util/bfloat16.h"  // NOLINT
#include "tensorstore/util/float8.h"    // NOLINT

// The implementation below is derived from jax-ml/ml_dtypes:
// https://github.com/jax-ml/ml_dtypes/blob/main/ml_dtypes/_src/ufuncs.h
...
std::pair<BitsType<T>, BitsType<T>> SignAndMagnitude(T x) {
  ...
  const BitsType<T> x_abs_bits = absl::bit_cast<BitsType<T>>(abs(x));
  const BitsType<T> x_bits = absl::bit_cast<BitsType<T>>(x);
...
struct NextAfter {
  T operator()(T from, T to) {
    BitsType<T> from_rep = absl::bit_cast<BitsType<T>>(from);
    BitsType<T> to_rep = absl::bit_cast<BitsType<T>>(to);
    ...
    if (from_abs == 0) {
      if (to_abs == 0) {
        ...
        return absl::bit_cast<T>(static_cast<BitsType<T>>(0x01 | to_sign));
      }
    }
    ...
    if constexpr (!std::numeric_limits<T>::is_iec559) {
      if (isnan(out)) {
        return absl::bit_cast<T>(BitsType<T>{0});
      }
    }
    return out;
  }
...
struct Spacing {
  T operator()(T x) {
    ...
    if constexpr (!std::numeric_limits<T>::has_infinity) {
      if (abs(x) == std::numeric_limits<T>::max()) {
...
#endif  // THIRD_PARTY_PY_TENSORSTORE_UFUNCS_H_
